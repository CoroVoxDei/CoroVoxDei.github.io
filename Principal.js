...
function initSearch() {
  ...
  const calculateSongScore = (song, query) => {
    ...
    const categoryId = song.dataset.type || "";
    const categoryName = catMap[categoryId] || "";
    const nCategory = normalize(categoryName);
    ...
    return { score, hasTitleMatch, hasAuthorMatch };
  };
  ...
  searchInput?.addEventListener("keyup", () => {
    ...
    const songsWithScore = allSongs.map(song => {
      ...
      return { song, score: res.score, hasTitleMatch: res.hasTitleMatch, hasAuthorMatch: res.hasAuthorMatch };
    });

    const matched = songsWithScore.filter(item => item.score > 0);
    const anyTitleOrAuthorMatched = matched.some(item => item.hasTitleMatch || item.hasAuthorMatch);

    let filteredSongsList = matched;
    if (anyTitleOrAuthorMatched) {
      // Si hay coincidencias en título o autor, limitamos la búsqueda solo a ellas para máxima especificidad
      filteredSongsList = matched.filter(item => item.hasTitleMatch || item.hasAuthorMatch);
    }
...
(function(){
  ...
  feedbackFab?.addEventListener("click", () => {
    ...
    const isPopupVisible = document.getElementById("popupOverlay")?.style.display === "flex";
