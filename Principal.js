...
function initSearch() {
  ...
  const fuzzyMatch = (text, query) => {
    ...
    // Si la búsqueda es exacta (sin acentos), aceptamos de inmediato
    if (nText.includes(nQuery)) return true;

    const queryWords = nQuery.split(/\s+/);
    const textWords = nText.split(/[\s,.\-!¡?¿]+/);

    // Cada palabra de la búsqueda debe tener al menos una coincidencia (exacta o similar)
    return queryWords.every(qWord => {
      ...
      return textWords.some(tWord => {
        ...
        // Coincidencia parcial o similar (distancia de Levenshtein)
  ...
  searchInput?.addEventListener("keyup", () => {
    ...
    allSongs.forEach(song => {
      ...
      const lyrics = song.querySelector(".lyrics, .lyrics1, .lyrics-hidden")?.textContent || "";
      const categoryId = song.dataset.type || "";
      const categoryName = catMap[categoryId] || "";
      
      const searchableText = `${title} ${lyrics} ${categoryName}`;
      song.style.display = fuzzyMatch(searchableText, filter) ? "block" : "none";
    });
  });
