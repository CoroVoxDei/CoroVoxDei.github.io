...
function filterByCategory(category) {
  ...
  if (category === "todos") {
    ...
    filteredSongs.sort((a, b) => {
      const titleA = (a.querySelector(".repertorio-title")?.textContent || 
                      a.querySelector("h2")?.childNodes[0]?.textContent || 
                      a.querySelector("h2 div")?.childNodes[0]?.textContent || "").trim().toLowerCase();
      const titleB = (b.querySelector(".repertorio-title")?.textContent || 
                      b.querySelector("h2")?.childNodes[0]?.textContent || 
                      b.querySelector("h2 div")?.childNodes[0]?.textContent || "").trim().toLowerCase();
  ...
  saved.forEach(rep => {
    ...
    card.innerHTML = `
      ...
      <div class="saved-repertorio-actions">
        <button class="btn-load-repertorio" title="Abrir este repertorio">
          <i data-lucide="folder-open"></i> <span class="btn-text">Abrir</span>
        </button>
    ...
    // Asignar eventos de forma segura con JS para evitar problemas de inyección
    card.querySelector(".btn-load-repertorio").onclick = () => window.abrirVerRepertorio(rep.id);
    card.querySelector(".btn-delete-repertorio").onclick = () => window.eliminarRepertorioGuardado(rep.id);
...
function getGlobalSongsList() {
  ...
  return window.allSongs.map(songSection => {
    ...
    return { title, author, lyrics, type, audio, tags, category };
  }).filter(s => s.title);
}
...
window.renderVerRepertorioDetalle = function() {
  ...
  if (target.songs.length === 0) {
    ...
    target.songs.forEach((song, idx) => {
      ...
      let cleanAuthor = (song.author || "").trim();
      while (cleanAuthor.startsWith("(") && cleanAuthor.endsWith(")")) {
        cleanAuthor = cleanAuthor.slice(1, -1).trim();
      }
      const displayedAuthor = cleanAuthor ? `(${cleanAuthor})` : "";

      infoA.innerHTML = `
        <i data-lucide="music" class="saved-rep-song-icon"></i>
    ...
    if (catNormal === "todos") {
      ...
      sortedCatKeys.forEach(catKey => {
        ...
        categories[catKey].sort((a, b) => {
          ...
          const li = document.createElement("li");
          li.innerHTML = `
            <a href="#" onclick="window.irACancion('${title.replace(/'/g, "\\'")}'); return false;">
              <span class="index-item-left">
                ${noteSvg}
                <span class="index-text-container">
                  <span class="index-song-title">${title}</span>
                  ${author ? `<span class="index-author">${author}</span>` : ""}
                </span>
              </span>
            </a>`;
          list.appendChild(li);
  ...
  window.irACancion = (titulo) => {
    const song = allSongs.find(s => {
      ...
      if (h2) {
        const h2Clone = h2.cloneNode(true);
        h2Clone.querySelectorAll("small, span, div, p").forEach(el => el.remove());
        t = h2Clone.textContent.trim();
      }
      if (!t) {
        t = s.querySelector("h2")?.childNodes[0]?.textContent.trim() || "";
      }
      return t.toLowerCase().trim() === titulo.toLowerCase().trim();
    });
