...
/* ========================
   4.1 SCROLL HORIZONTAL CON FLECHAS
======================== */
const catScroll = document.getElementById("catScroll");
const catLeft = document.getElementById("catLeftBtn");
const catRight = document.getElementById("catRightBtn");
...
/* ==============================
   7. MOSTRAR / OCULTAR ACORDES (Lógica Global)
============================== */
...
window.renderPopupLyrics = function() {
    ...
    if (window.showChords) {
        ...
        lines.forEach(line => {
            const temp = document.createElement("div");
            temp.innerHTML = line;
            const chordSpans = temp.querySelectorAll(".chord");
            const hadChords = chordSpans.length > 0;
            chordSpans.forEach(c => c.remove());
...
function updateChordsVisibility() {
  ...
  if (btn) btn.textContent = window.showChords ? "Ocultar acordes" : "Mostrar acordes";
  const popup = document.getElementById("popupLetra");
  if (popup && popup.style.display !== "none") window.renderPopupLyrics();
  localStorage.setItem("showChords", window.showChords);
}

// Solo activar el listener en index.html para evitar duplicados en Repertorio
if (document.body.id !== "repertorio-page" && !window.location.pathname.includes("Repertorio.html")) {
    document.getElementById("toggleChordsBtn")?.addEventListener("click", e => {
      e.preventDefault();
...
function initSongButtons() {
  document.querySelectorAll(".add-repertorio").forEach(btn => {
    ...
    const title = songSection.querySelector("h2")?.childNodes[0]?.textContent.trim() || "";
    let repertorio = JSON.parse(localStorage.getItem("repertorio")) || [];
    actualizarBoton(btn, repertorio.some(s => s.title === title));

    btn.addEventListener("click", () => {
      let rep = JSON.parse(localStorage.getItem("repertorio")) || [];
      const index = rep.findIndex(s => s.title === title);
      if (index === -1) {
        const author = songSection.querySelector(".autor")?.textContent.trim() || "";
        const lyrics = songSection.querySelector(".lyrics-hidden")?.innerHTML.trim();
        rep.push({ title, author, lyrics });
        localStorage.setItem("repertorio", JSON.stringify(rep));
        actualizarBoton(btn, true);
      } else if (confirm(`¿Deseas eliminar "${title}" del repertorio?`)) {
        rep.splice(index, 1);
        localStorage.setItem("repertorio", JSON.stringify(rep));
        actualizarBoton(btn, false);
      }
...
// Solo cargar canciones si estamos en index
...
(function(){
  ...
  document.addEventListener("click", e => {
    ...
    const h2 = btn.closest(".song")?.querySelector("h2");
    const titulo = h2?.childNodes[0]?.textContent?.trim() || btn.dataset.title || "";
    const autor = h2?.querySelector(".autor")?.textContent?.trim() || btn.dataset.author || "";
    abrirLetra(titulo, letraHtml, autor);
  });
...
window.cambiarFuente = function(delta) {
    ...
    if (letra) {
        ...
        letra.querySelectorAll("span").forEach(span => span.style.fontSize = window.tamañoFuente + "px");
    }
};
    window.tamañoFuente += delta;
