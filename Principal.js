...
// Función para barajar el array de canciones
...
function loadSongs(files) {
  ...
  Promise.all(files.map(f => fetch(f).then(r => r.text()).catch(() => "")))
    .then(htmls => {
      ...
      // Aleatoriedad: barajamos la lista de canciones antes de mostrarlas
...
/* ===============================
   11. POPUP DE LETRAS & AJUSTES (REDISEÑADO)
=============================== */
(function(){
  ...
  function resetHideTimer() {
    // Si el menú de ajustes está abierto, no ocultamos el bloque principal
  ...
  // Toggle Menú Principal (Engranaje)
  settingsBtn?.addEventListener("click", (e) => {
    ...
    if (!isActive) {
        // Cerrar paneles abiertos si se cierra el engranaje
        panelSize?.classList.remove("active");
        panelSpacing?.classList.remove("active");
        btnToolSize?.classList.remove("active");
        btnToolSpacing?.classList.remove("active");
