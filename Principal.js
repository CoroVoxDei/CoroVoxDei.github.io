...
function renderizarRepertorio(lista, esBusqueda = false) {
    ...
    lista.forEach((song, index) => {
      ...
      // Determinar etiqueta de tipo (YA NO SE MUESTRA EN LA LISTA PRINCIPAL)
      let tagHtml = "";
      ...
      section.innerHTML = `
        <div class="song-header">
          ...
          <div class="song-btns">
            <button class="lyrics-btn">Ver letra</button>
...
function addDragEvents(item) {
  ...
  if (handle) {
    handle.addEventListener('touchstart', function(e) {
      // Indicamos visualmente que se ha seleccionado para arrastrar
      item.classList.add('dragging');
    }, { passive: true });

    handle.addEventListener('touchmove', function(e) {
      // Bloqueamos el scroll de la página para que el dedo mueva la tarjeta
      if (e.cancelable) e.preventDefault();
      
      const touch = e.touches[0];
      // Localizamos qué elemento está debajo del dedo actualmente
      const elementUnderTouch = document.elementFromPoint(touch.clientX, touch.clientY);
      const songTarget = elementUnderTouch?.closest('.song');
      
      // Limpiamos estados 'over' de todas las canciones
      document.querySelectorAll('.song').forEach(s => s.classList.remove('over'));
      
      // Si el dedo está sobre otra canción válida del repertorio, mostramos feedback
      if (songTarget && songTarget !== item && songTarget.closest('#repertorio-list')) {
        songTarget.classList.add('over');
      }
    }, { passive: false });
    ...
    handle.addEventListener('touchend', function(e) {
      ...
      // Para saber dónde cayó, ocultamos momentáneamente la tarjeta que arrastramos
      item.style.visibility = 'hidden';
      const elementAtEnd = document.elementFromPoint(touch.clientX, touch.clientY);
      item.style.visibility = 'visible';
      
      const songTarget = elementAtEnd?.closest('.song');
      document.querySelectorAll('.song').forEach(s => s.classList.remove('over'));

      if (songTarget && songTarget !== item && songTarget.closest('#repertorio-list')) {
        const fromIdx = parseInt(item.dataset.index);
        const toIdx = parseInt(songTarget.dataset.index);
  ...
  document.querySelectorAll(".add-repertorio").forEach(btn => {
    ...
    btn.onclick = (e) => {
      ...
      if (index === -1) {
        ...
        currentRep.push({ title, author, lyrics, type });
...
(function(){
  ...
  window.abrirLetra = function(titulo, letraHtml, autor, tipo) {
    const popup = document.getElementById("popupLetra");
    if (!popup) return;
  ...
  document.addEventListener("click", e => {
    ...
    let title = "";
    if (h2.querySelector("div")) {
        // Estructura de Repertorio (con div contenedor de título)
    ...
    window.abrirLetra(title, lyrics, autor, tipo);
  });
  ...
  window.irACancion = (titulo) => {
    ...
    if (song) {
      ...
      window.abrirLetra(titulo, lyrics, autor, tipo);
      window.cerrarIndice();
    }
  };
